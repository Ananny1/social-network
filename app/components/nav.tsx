...
export default function Nav({ isDarkMode }) {
  ...
  useEffect(() => {
    // GSAP animation for rotating the flower
  ...
  useEffect(() => {
    if (searchTerm.trim() === '' || !Array.isArray(users) || users.length === 0) {
      setFilteredUsers([]);
    } else {
  ...
  return (
    <nav className={`navbar shadow-lg ${isDarkMode ? 'bg-gray-800 text-gray-100' : 'bg-base-100 text-gray-900'}`}>
      <div className="flex-1">
        <button className="btn btn-ghost normal-case text-xl" onClick={handleHome}>
          S
          <span ref={flowerRef} className="inline-block mx-1" style={{ display: 'inline-block', width: '2em', height: '2em', transformOrigin: 'center center', marginLeft: '-0.70em', marginRight: '-0.70em' }}>
            {/* Flower SVG */}
